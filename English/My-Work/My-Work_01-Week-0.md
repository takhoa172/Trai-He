# Tuần 0

---
## Ngày 1 - Diagnostic Test

### Bài 1: Trắc nghiệm (10 câu)

| STT | Đáp án chọn | Đáp án đúng | Kết quả    |
| :-: | :---------: | :---------: | :--------- |
|  1  |      a      |      A      | ***Đúng*** |
|  2  |      a      |      B      | ***Sai***  |
|  3  |      b      |      B      | ***Đúng*** |
|  4  |      b      |      B      | ***Đúng*** |
|  5  |      a      |      A      | ***Đúng*** |
|  6  |      c      |      B      | ***Sai***  |
|  7  |      b      |      B      | ***Đúng*** |
|  8  |      d      |      B      | ***Sai***  |
|  9  |      c      |      C      | ***Đúng*** |
| 10  |      a      |      B      | ***Sai***  |
=> **Kết quả:** *6/10 đúng*

---

### ✍️ Bài 2: Viết lại câu (VN → EN)

| #   | Câu gốc               | Tôi viết                 | Đáp án đúng                | Lỗi            |
| --- | --------------------- | ------------------------ | -------------------------- | -------------- |
| 1   | Tôi là học sinh       | I am student             | I am **a** student         | ❌ Thiếu "a"    |
| 2   | Bạn tên là gì?        | What your name?          | What **is** your name?     | ❌ Thiếu "is"   |
| 3   | Cảm ơn bạn            | Thank you                | Thank you                  | ✅              |
| 4   | Tôi xin lỗi           | I am sorry               | I am sorry                 | ✅              |
| 5   | Tạm biệt, hẹn gặp lại | Good bye, see you latter | Goodbye, see you **later** | ❌ Sai chính tả |

---

### 💬 Bài 3: Điền từ

| # | Câu gốc | Tôi điền | Đáp án | Kết quả |
|---|---------|---------|--------|---------|
| 1 | "___" when you meet someone | hello | Hello | ✅ |
| 2 | "___", I don't understand | no | **Sorry** | ❌ |
| 3 | "___ you" after someone helps | thank you | Thank | ✅ |
| 4 | "___" when you leave | sorry | **Goodbye** | ❌ |
| 5 | Can you help me, "___"? | please | please | ✅ |

---

### 🔤 Bài 4: Sắp xếp từ

| # | Tôi viết | Đáp án | Lỗi |
|---|----------|--------|-----|
| 1 | John is my name | My name is John | ❌ Trật tự từ |
| 2 | How are you | How are you? | ✅ |
| 3 | sit down please | Sit down, please | ❌ Viết hoa + phẩy |
| 4 | I am late sorry | Sorry, I am late | ❌ Sai thứ tự |
| 5 | Nice to meet you | Nice to meet you | ✅ |

---

### 📝 Bài 5: Tự đánh giá

- **Phần khó nhất:** Kiến thức nền tảng (to be, thì, sở hữu, giới từ, mạo từ) — cái nào cũng biết chút nhưng chưa vững.
- **Mong đợi:** Nắm vững nền tảng, có lộ trình thú vị và hiệu quả.

---

## 📅 Ngày 2 — Continue Diagnostic

### 👂 Bài 1: Trạm Nghe (Listening)

| #   | Câu gốc                     | Tôi nghe                   | Đúng | Lỗi                       |
| --- | --------------------------- | -------------------------- | ---- | ------------------------- |
| 1   | Hello, my name is Anna.     | Hello my name is Anna      | ✅    | Thiếu phẩy                |
| 2   | Please open your book.      | Please show open your book | ❌    | Thừa "show"               |
| 3   | Thank you for the pen.      | Thank you for a pen        | ❌    | Sai mạo từ (a → the)      |
| 4   | Sorry, this is my desk.     | sorry this is my desk      | ✅    | Thiếu viết hoa + phẩy     |
| 5   | Goodbye, see you on Monday. | Good bye see you on Monday | ❌    | Sai chính tả + thiếu phẩy |

### ✍️ Bài 2: Trạm Viết (Writing)

| #   | Tôi viết                      | Đúng | Lỗi                                            |
| --- | ----------------------------- | ---- | ---------------------------------------------- |
| 1   | I have a red pen              | ✅    |                                                |
| 2   | My desk is very small         | ✅    |                                                |
| 3   | I usually read book every day | ❌    | Thiếu "a" → **read a book**                    |
| 4   | I writer in the board         | ❌    | "writer" → **write**, "in" → **on**            |
| 5   | (bỏ trống)                    | ❌    | Gợi ý: *I have a white board in my classroom.* |

### 🔊 Bài 3: Trạm Phát Âm

| Từ     | IPA        | Tự đánh giá (1-5) |
| ------ | ---------- | :---------------: |
| book   | /bʊk/      |         4         |
| pen    | /pen/      |         4         |
| paper  | /ˈpeɪpər/  |         4         |
| desk   | /desk/     |         4         |
| chair  | /tʃɛər/    |         4         |
| board  | /bɔːrd/    |         2         |
| eraser | /ɪˈreɪsər/ |         2         |

### 🧠 Bài 4: Xác định lỗ hổng

1. **Nghe:** Nghe không rõ, ít lặp lại được, dễ quên.
2. **Cấu trúc câu:** Chưa nắm vững, không biết viết thế nào.
3. **Từ vựng:** Vốn ít, không biết đặt câu hoặc sử dụng từ.

### 🔗 Bài 5: Ôn từ vựng chéo

| #   | Ghép            | Kết quả |
| --- | --------------- | ------- |
| 1   | book — d. sách  | ✅       |
| 2   | pen — a. bút    | ✅       |
| 3   | paper — f. giấy | ✅       |
| 4   | desk — g. bàn   | ✅       |
| 5   | chair — b. ghế  | ✅       |
| 6   | board — c. bảng | ✅       |
| 7   | eraser — e. tẩy | ✅       |

**Kết quả: 7/7 đúng** ✅

---

## 📅 Ngày 3 — Sentence Garden Setup

### 📝 Bài 1: Tạo Sentence Garden

✅ Đã tạo file `Study-Notebook/Sentence-Garden.md` với cấu trúc 5 lớp.

---

### 🌿 Bài 2: Viết 5 câu giới thiệu bản thân

| # | Câu tôi viết | Meaning | Form (tôi) | Form (đúng) | Sound | Use | Review | Lỗi |
|---|-------------|---------|-----------|-------------|-------|-----|--------|-----|
| 1 | My name is Khoa | Tên tôi là Khoa | S+V+N | S + am/is/are + N | /maɪ neɪm ɪz kwaː/ | Giới thiệu tên | 2026-07-25 | ❌ Form chưa chi tiết |
| 2 | I am a student | Tôi là học sinh | S+V+N | S + am/is/are + a/an + N | /aɪ æm ə ˈstuːdənt/ | Nói về nghề nghiệp | 2026-07-25 | ❌ Form chưa chi tiết |
| 3 | I am from Vietnam | Tôi đến từ Việt Nam | S+V+N | S + am/is/are + from + N | /aɪ æm frɒm ˌviɛtˈnæm/ | Nói về quê quán | 2026-07-25 | ❌ Form chưa chi tiết, chính tả "Vietnam" |
| 4 | I am 21 years old | Tôi 21 tuổi | S+V+N | S + am/is/are + số + years old | /aɪ æm ˈtwɛnti wʌn jɪərz oʊld/ | Nói về tuổi tác | 2026-07-25 | ❌ Form chưa chi tiết |
| 5 | Nice to meet you | Rất vui được gặp bạn | S+V+N | Câu cố định | /naɪs tə miːt juː/ | Chào hỏi lần đầu gặp | 2026-07-25 | ❌ Form + Use sai |

### ❓ Bài 3: Wh-questions

| # | Câu hỏi | Tôi điền | Đáp án | Kết quả |
|---|---------|---------|--------|---------|
| 1 | ___ are you? — I am fine. | How | How | ✅ |
| 2 | ___ is your name? — My name is Hoa. | What | What | ✅ |
| 3 | ___ are you from? — I am from Hanoi. | Where | Where | ✅ |
| 4 | ___ is he? — He is my friend. | Who | Who | ✅ |
| 5 | ___ is your phone number? — It is 0123456789. | What | What | ✅ |

**Kết quả: 5/5 đúng** ✅

---

### ✍️ Bài 4: Dịch câu (VN → EN)

| # | Tiếng Việt | Tôi dịch | Đáp án | Kết quả |
|---|-----------|---------|--------|---------|
| 1 | Tên tôi là Lan. | My name is Lan. | My name is Lan. | ✅ |
| 2 | Bạn khỏe không? | How are you? | How are you? | ✅ |
| 3 | Bạn đến từ đâu? | Where are you from? | Where are you from? | ✅ |
| 4 | Ai là giáo viên của bạn? | Who is your teacher? | Who is your teacher? | ✅ |
| 5 | Tôi 20 tuổi. | I am 20 years old. | I am 20 years old. | ✅ |

**Kết quả: 5/5 đúng** ✅

---

### 💬 Bài 5: Sắp xếp hội thoại

| Thứ tự tôi sắp xếp | Thứ tự đúng |
|:---:|:---:|
| C → F → A → D → B → E | C → F → E → B → A → D |

**Kết quả:** ❌ Sai — chào hỏi lịch sự (Nice to meet you) phải trước khi hỏi quê quán.

**Thứ tự đúng:**
1. C: Hello, my name is John.
2. F: Hi John. I'm Minh.
3. E: Nice to meet you, Minh.
4. B: Nice to meet you too, John!
5. A: I am from Hanoi. Where are you from?
6. D: I am from Ho Chi Minh City.

---

## 📅 Ngày 4 — Set Up Vocabulary Rooms

### 📝 Bài 1: Tạo Vocabulary Rooms

✅ Đã tạo file `Study-Notebook/Vocabulary-Rooms.md` với 4 phòng:
- 🏠 Me (13 từ)
- 🏠 Family (chờ điền)
- 🏠 School (7 từ)
- 🏠 Numbers (7 từ)

---

### 🔢 Bài 2: Mở rộng câu với số đếm

| #   | Câu                                 | Tôi điền | Đáp án               | Kết quả              |
| --- | ----------------------------------- | -------- | -------------------- | -------------------- |
| 1   | There are ___ days in a week.       | seven    | Seven                | ✅                    |
| 2   | I have ___ hands.                   | one      | **two**              | ❌                    |
| 3   | A triangle has ___ sides.           | 3        | **three**            | ❌ Ghi số thay vì chữ |
| 4   | A week has ___ Mondays.             | one      | one                  | ✅                    |
| 5   | There are ___ chairs and ___ table. | five     | **four** and **one** | ❌                    |

---

### 📦 Bài 3: Sắp xếp từ vào phòng

| Me 🏠 | Family 🏠 | School 🏠 |
|-------|----------|----------|
| hello | mother | book |
| goodbye | father | pen |
| name | sister | eraser |
| thank you | brother | desk |
| sorry | | chair |
| | | board |

✅ Các từ đã được thêm vào `Vocabulary-Rooms.md` tương ứng.

---

### 🌿 Bài 4: Thêm 3 câu vào Sentence Garden

| #   | Câu tôi viết                 | Câu đúng                      | Kết quả                 |
| --- | ---------------------------- | ----------------------------- | ----------------------- |
| 1   | My family have four number   | My family has four people     | ❌ Sai have/has + sai từ |
| 2   | I have one dog               | I have one dog                | ✅                       |
| 3   | There are six pen on my desk | There are six pens on my desk | ❌ Thiếu "s" số nhiều    |

---

### 🎲 Bài 5: Bingo số

| # | Câu | Tôi điền | Đáp án | Kết quả |
|---|-----|---------|--------|---------|
| 1 | I am twenty-___ years old. | one | twenty-one | ✅ |
| 2 | There are ___ people in my family. | four | four | ✅ |
| 3 | I have ___ apples. | three | three | ✅ |
| 4 | She needs ___ pens. | two | two | ✅ |
| 5 | We have ___ cats. | one | one | ✅ |
| 6 | There are ___ books on the desk. | serven | **seven** | ❌ Sai chính tả |
| 7 | He is ___ years old. | threety-two | **thirty-two** | ❌ Sai chính tả |

---

### ✅ Tự kiểm tra cuối ngày

1. **3 phòng từ vựng đã tạo:** Me (13 từ), Family (4 từ), School (7 từ), Numbers (7 từ)
2. **Số đếm 1-7:** one, two, three, four, five, six, seven
3. **There is/are:** There is a computer on my desk.

---

## 📅 Ngày 5 — Error Ledger & Review Calendar

### 📕 Bài 1: Tạo Error Ledger

✅ Đã tạo file `Study-Notebook/Error-Ledger.md` với 7 lỗi từ Days 1-4:

| Ngày | Lỗi |
|------|-----|
| Day 1 | Thiếu "a/an", thiếu "is", sai trật tự từ |
| Day 2 | Sai "a→the", sai "writer in→write on" |
| Day 4 | Quên "s" số nhiều, sai "have/has" |

---

### 📆 Bài 2: Tạo Review Calendar

✅ Đã tạo file `Study-Notebook/Review-Calendar.md` với lịch ôn Tuần 0 (Day 1→7).

---

### 🎨 Bài 3: Mô tả màu sắc

| # | Câu gốc | Tôi viết | Đáp án | Kết quả |
|---|---------|---------|--------|---------|
| 1 | The sky ___ | Blue | blue | ✅ |
| 2 | My pen ___ | Black | black | ✅ |
| 3 | The sun ___ | Yellow | yellow | ✅ |
| 4 | The flowers ___ | Pink | pink | ✅ |
| 5 | This book ___ | Red | red | ✅ |

**Kết quả: 5/5 đúng** ✅

---

### ✏️ Bài 4: Điền từ màu sắc

| # | Câu gốc | Tôi điền | Đáp án | Kết quả |
|---|---------|---------|--------|---------|
| 1 | An apple is ___ or green | red or green | red or green | ✅ |
| 2 | A banana is ___ | Yellow | yellow | ✅ |
| 3 | Snow is ___ | white | white | ✅ |
| 4 | Night sky is ___ | Black | black | ✅ |
| 5 | Grass is ___ | green | green | ✅ |

**Kết quả: 5/5 đúng** ✅

---

### 🔧 Bài 5: Tìm và sửa lỗi

| # | Câu sai | Tôi sửa | Đáp án | Kết quả |
|---|---------|---------|--------|---------|
| 1 | She go to school | goes | goes | ✅ |
| 2 | I are a student | am | am | ✅ |
| 3 | He have a book | has | has | ✅ |
| 4 | There is three cats | are | are | ✅ |
| 5 | My name are John | is | is | ✅ |

**Kết quả: 5/5 đúng** ✅

---

## 📅 Ngày 6 — Reading & Imitation Practice

### 📖 Bài 1: Đọc hiểu — Lisa

| # | Câu hỏi | Tôi trả lời | Đáp án | Kết quả |
|---|---------|------------|--------|---------|
| 1 | What is her name? | Lisa | Her name is Lisa | ⚠️ Thiếu "Her name is" |
| 2 | How old is she? | Twenty years old | **22** years old | ❌ Sai số |
| 3 | Where is she from? | From Vietnam | She is from Vietnam | ✅ |
| 4 | What color is her book? | Red | Her book is red | ✅ |
| 5 | What is her favorite color? | Pink | Her favorite color is pink | ✅ |
| 6 | When does she go to school? | Monday to Friday | From Monday to Friday | ✅ |
| 7 | What does she do on Sat & Sun? | Stay at home | She stays at home | ✅ |

---

### 🔤 Bài 4: Đoạn văn giới thiệu bản thân

| Câu | Tôi viết | Đúng | Lỗi |
|-----|---------|:----:|-----|
| 1 | Hello! My name is Khoa | ✅ | |
| 2 | I am a student | ✅ | |
| 3 | I am twenty years old | ✅ | |
| 4 | I am from VietNam | ⚠️ | → Vietnam |
| 5 | I have a red pen and a white book | ✅ | |
| 6 | I favorite play game | ❌ | → My hobby is playing games |
| 7 | I fell good and happy | ❌ | → I feel happy |

---

### 📅 Bài 5: Điền thứ trong tuần

| # | Câu gốc | Tôi điền | Đáp án | Kết quả |
|---|---------|---------|--------|---------|
| 1 | The first day of school week is ___ | Monday | Monday | ✅ |
| 2 | The day after Tuesday is ___ | Wednday | **Wednesday** | ❌ Sai chính tả |
| 3 | The last day of the week is ___ | Sunday | Sunday | ✅ |
| 4 | The day before Friday is ___ | Thurday | **Thursday** | ❌ Sai chính tả |
| 5 | TGIF — Thank God It's ___! | No problem | **Friday** | ❌ Sai |

---

### ✅ Tự kiểm tra cuối ngày

1. **7 thứ trong tuần:** Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday
2. **Đoạn văn:** Cần sửa 3 lỗi (Vietnam, hobby, feel)
3. **5 lớp:** Meaning (1), Form (2), Sound (3), Use (4), Review (5)


